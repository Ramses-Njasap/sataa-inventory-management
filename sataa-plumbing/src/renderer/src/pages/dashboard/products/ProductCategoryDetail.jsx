...
//   }, []);
  ...
  const fetchCategoryData = useCallback(async () => {
    try {
      ...
      const currentCategory = categoryData.find(cat => cat.id === Number(id));
    ...
    //     const fullImagePath = await window.api.getImageFullPath(currentCategory.image_path);
...
//   };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await window.api.updateProductCategory({
        id: Number(id),
        name: formData.name,
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        setLoading(true);
        await window.api.deleteProductCategory(Number(id));
        navigate('/dashboard/product-categories');
      } catch (err) {
  ...
  if (!category) {
    return (
      <div className="text-slate-900 p-6">
        <p className="text-rose-400">Failed to load category</p>
        <button 
  ...
  return (
    <div className="bg-slate-50 p-6 rounded-xl shadow-sm border border-slate-300">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-slate-900 text-lg font-semibold">Edit Category: {category.name}</h3>
